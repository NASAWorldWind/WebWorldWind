...
define([
    ...
    function (Angle,
        ...
        ProjectionWgs84.prototype.cartesianToLocalTransform = function (globe, x, y, z, result) {
            if (!globe) {
                throw new ArgumentError(Logger.logMessage(Logger.LEVEL_SEVERE, "ProjectionWgs84", "cartesianToLocalTransform", "missingGlobe"));
            }

            if (!result) {
                throw new ArgumentError(Logger.logMessage(Logger.LEVEL_SEVERE, "ProjectionWgs84", "cartesianToLocalTransform", "missingResult"));
            }

            var pos = this.cartesianToGeographic(globe, x, y, z, Vec3.ZERO, this.scratchPosition);
            var radLat = pos.latitude * Angle.DEGREES_TO_RADIANS;
            var radLon = pos.longitude * Angle.DEGREES_TO_RADIANS;
            var cosLat = Math.cos(radLat);
            var sinLat = Math.sin(radLat);
            var cosLon = Math.cos(radLon);
            var sinLon = Math.sin(radLon);
